...
async function dcExport(node, w, h, name, kind) {
  ...
  // Inline any same-document icon sprite so <use href="#id"> resolves inside
  // the standalone SVG (the sprite lives outside the artboard, so otherwise
  // icons would export blank). Cheap and safe: only runs if the card uses <use>.
  if (clone.querySelector && clone.querySelector('use')) {
    const sprites = document.querySelectorAll('#sprite svg, svg#sprite');
    sprites.forEach((sp) => {
      const holder = document.createElement('div');
      holder.setAttribute('style', 'position:absolute;width:0;height:0;overflow:hidden;pointer-events:none');
      holder.appendChild(sp.cloneNode(true));
